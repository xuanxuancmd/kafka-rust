...
/// Indicates that an operation attempted to modify or delete a connector or task
/// that is not present on the worker.
///
/// This corresponds to `org.apache.kafka.connect.errors.NotFoundException` in Java.
#[derive(Debug)]
...
impl NotFoundException {
    ...
    /// Creates a new NotFoundException with a message and a cause.
    ...
    /// Creates a new NotFoundException from a cause.
...
impl fmt::Display for NotFoundException {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(f, "NotFoundException: {}", self.inner.message())
    }
}
...
impl From<ConnectException> for NotFoundException {
    fn from(inner: ConnectException) -> Self {
        NotFoundException { inner }
    }
}
